...
function validate_mobile($dmobile) {
    return preg_match('/^[0]?(1)[3456789]\d{8}$/', $dmobile);
}
    ...
    if ($userActive['active'] == 1) {
        ...
        if (isset($_POST['match'])) {
            ...
            if (isset($_COOKIE['club'])) {
				...
				$query = "SELECT * FROM `betting_title` WHERE id='$matchId' and STR_TO_DATE(dateend,'%d %b %Y %h:%i %p') >= NOW() 								ORDER BY STR_TO_DATE(date,'%d %b %Y %h:%i %p') ASC";
				$checkdate = $db->select($query);
                ...
                if ($match != "" && $matchBet != "" && $betRate != "" && $stakeAmount != "" && $betId != "") {
                    ...
                    if ($active) {
                        if ($stakeAmount > 0.00) {
                            if ($stakeAmount >= 20.00 && $stakeAmount <= 10000.00) {
                                $ok = 1;
                                //limit check match
                                $query = "SELECT * FROM `betting_title` WHERE id='$matchId'";
                                $result = $db->select($query);
                                ...
                                echo ' Minimum bet amount 20  and maximum amount 10000 !! ';
                            }
                        } else {
					...
					echo "Match Closed. You cannot bet now!!";
				}
            }
        }
//deposit submit
            ...
            if ($dAmount != "" && $dMethodt != "" && $dFrom != "" && $dReference != "" && $dTo != "") {
                ...
                if ($dAmount > 99.0 && $dAmount < 50001) {
					
					if(validate_mobile($dFrom)){

                    if (isset($_COOKIE["userId"]) && isset($_COOKIE["password"]) && isset($_COOKIE["id"])) {
                        //previous submission check
						$query = "SELECT * FROM `admin_notification` WHERE userId='$_COOKIE[userId]' and notificationType=1 and seen=0 and action=1 ORDER BY id desc LIMIT 1";
                        $resultBalance = $db->select($query);
					...
					echo "Please Check Mobile no.!!!";
				}
                } else {
                    echo 'Please Insert Valid Amount. Please Insert Valid Amount or You cant deposit less than 100tk or bigger than 50000tk !! ';
                }
            } else {
            ...
            if (isset($_COOKIE['userId'])) {
                ...
                if (isset($_COOKIE['club'])) {
                    ...
                    if ($wAmount != "" && $password != "") {
                        ...
                        if ($wAmount > 0.0) {
                            ...
                            if ($resultBalanceCheck) {
								...
								echo "You cant withdraw less than 1000 and upper than 5000";
							}
                        } else {
                    ...
                    $transferPassword = md5($_POST['wPassword']);
                                        ...
                                        if ($wAmount != "" && $id != "" && $wType != "" && $wTo != "") {
                                            ...
                                            if ($wAmount > 0.0) {
                                                ...
                                                if ($resultBalanceCheck) {
                                                    ...
                                                    $query = "SELECT * FROM `rule`";
                                                    $resultRule = $db->select($query);
                                                    $minBalance = $resultRule->fetch_assoc();
                                                    $minimumBalance = $minBalance['minimumBalance'];
													$maximumBalance = $minBalance['wmaximumBalance'];
													$wminimumBalance = $minBalance['wminimumBalance'];

                                                    if (($wAmount * $rate) <= ($balance['balance'] - $minimumBalance)) {
                                                        if (($wAmount >= $wminimumBalance) && ($wAmount <= $maximumBalance)) {
                                                        ...
                                                        echo "minimum withdraw balance $wminimumBalance and maximum withdraw balance $maximumBalance";
                                                    }
                                                }
                                                else
                                                {
                                                    echo 'Not sufficient fund!! please deposit. ';
                                                }
                                                } else {
                                                    echo 'Create Account';
                                                }
												...
												echo "Please check your mobile no.!!!";
											}
                                        } else {
                                    ...
                                    if ($wAmount != "" && $id != "" && $wType != "" && $wTo != "") {
                                        ...
                                        if ($wAmount > 0.0) {
                                            ...
                                            if ($resultBalanceCheck) {
                                                ...
                                                $minBalance = $resultRule->fetch_assoc();
                                                $minimumBalance = $minBalance['minimumBalance'];
												$wmaximumBalance = $minBalance['wmaximumBalance'];
                                                $wminimumBalance = $minBalance['wminimumBalance'];

                                                if (($wAmount * $rate) <= ($balance['balance'] - $minimumBalance)) {
												if ($wAmount >= $wminimumBalance && $wAmount<= $wmaximumBalance) {
                                                    if (isset($_COOKIE["userId"]) && isset($_COOKIE["password"]) && isset($_COOKIE["id"])) {
													...
													{
														echo "Minimum withdraw amount is $wminimumBalance and maximum withdraw limit $wmaximumBalance";
													}
                                                } else {
                                                    echo 'Not sufficient Balance minimum balance required ' . $minimumBalance . '!! please deposit. ';
                                                }
                                            } else {
                                                echo 'Create Account';
                                            }
            ...
            $transferPassword = md5($_POST['transferPassword']);
            $query = "select * from `user` where userId='$_COOKIE[userId]' and  password='$transferPassword'";
            if ($db->select($query)) {
            ...
            $cClub = $_POST['cClub'];
            $PasswordClubChange = md5($_POST['PasswordClubChange']);
