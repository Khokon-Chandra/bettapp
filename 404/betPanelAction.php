        ...
        if ($resultBetWin) {
            foreach ($resultBetWin as $win) {
                ...
                if ($db->select($query)) {
                    ...
                    $betstatusby="Wined by ".$admin." at ".$datetime; 

                    $query = "update `bet` set betStatus='1',actionAt='$dt',betstatusby='$betstatusby' WHERE id='$win[id]' and userId='$win[userId]' and betStatus=0 and betId='$ans' and action=0 and betTitleId='$que'";
                    $result = $db->update($query);
                    if ($result) {
                        $ok = 1;
        ...
        if ($resultBetLose) {
            foreach ($resultBetLose as $lose) {
                ...
                if ($db->select($query)) {
                    ...
                    $betstatusby="Wined by ".$admin." at ".$datetime; 
                    //update bet
                    $query = "update `bet` set betStatus='2',betstatusby='$betstatusby' WHERE userId='$lose[userId]' and betStatus=0 and betStatus='0' and action=0 and betTitleId='$que'";
                    $result = $db->update($query);
