...
if (isset($_POST['match_id'])) {
...
else if (isset($_POST['closedToPanel'])) {
    $matchId = $_POST['closedToPanel'];
    if ($matchId != '') {

        $query = "update betting_title set close=0 where id='$matchId'";
        $result = $db->update($query);
    }
}
else if (isset($_POST['closedquestinToPanel'])) {
    $qid = $_POST['closedquestinToPanel'];
    if ($qid != '') {

        $query = "update betting_subtitle set close=0 where id='$qid'";
        $result = $db->update($query);
    }
}
	...
	$datee =$dtend->format('d M Y');
	$timeend = $_POST['end_time'];
	$dateend= $datee." ".$timeend;
    ...
    if ($A_team != '' && $B_team != '' && $title != '' && $date != '' && $status != '' && $gameType != '') {
        ...
        $query = "INSERT INTO betting_title(A_team,B_team,title,date,dateend,status,gameType,user,color_a,color_b,dates,adminId)VALUES ('$A_team','$B_team','$title','$date','$dateend','$status','$gameType','$operator','$color_a','$color_b','$dates','$adminId')";
    ...
    if ($status != '' && $gameType != '') {
            ...
            $query = "INSERT INTO betting_title(`A_team`, `B_team`, `title`, `date`, dateend, `status`, `gameType`,user,dates,adminId)
            VALUES ('$A_team','$B_team','$title','$date', '$dateend', '$status','$gameType','$operator','$dates','$adminId')";
            $resultClubInsert = $db->insert($query);
            // select match id
            ...
            $query = "SELECT * FROM `default_ques` WHERE g_type='$gameType'";
            ...
            if ($resultBettingSubTitle) {
                while ($bettingSubTitle = $resultBettingSubTitle->fetch_assoc()) {
                    ...
                    $query = "INSERT INTO betting_subtitle( `title`, `bettingId`,section_ct,section_hide)
                            VALUES ('$bettingSubTitle[title]','$bettingTitleId[id]','$bettingSubTitle[section_ct]','1')";
                    $resultClubInsert = $db->insert($query);
                    // select question id
    ...
    $A_team = $_POST['A_team'];
    $color_aup=$_POST['color_aup'];
    $B_team = $_POST['B_team'];
    $color_bup=$_POST['color_bup'];
    $title = $_POST['title'];
    ...
    $ans=$_POST['editAnswer'];
    $editBetRate = $_POST['editRateAmount'];

    $BettingSubTitleOptionId = $_POST['ans_id_for_update'];
    if ($editBetRate != '') {

        $query = "update `betting_sub_title_option` set title='$ans', amount='$editBetRate' WHERE id='$BettingSubTitleOptionId'";
        $result = $db->update($query);
        ?>
    ...
    else if (isset($_POST['cancelMatch'])) {
    $cancelMatch = $_POST['cancelMatch'];
    if ($cancelMatch != '') {
          $query = "select userId,betAmount from bet where matchId='$cancelMatch'";
        ...
        while($row=mysqli_fetch_array($run))
        {
            ...
            $update = "update user set balance=balance+$balance where userId='$userId'";
            $update1 = "update bet set betAmount=0 where matchId='$cancelMatch'";
            mysqli_query($con,$update);
            mysqli_query($con,$update1);

        }
        $query = "update `betting_title` set showStatus='0',cancel=1 WHERE id='$cancelMatch'";
       $result = $db->update($query);
    }
    ...
    $query = "update `betting_title` set showStatus='1',cancel=0 WHERE id='$match_id'";
    $result = $db->update($query);
